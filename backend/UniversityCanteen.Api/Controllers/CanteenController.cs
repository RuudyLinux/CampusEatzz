    ...
    ILogger<CanteenController> logger) : ControllerBase
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var from = DateTime.TryParse(fromDate, out var fd) ? fd.Date : DateTime.UtcNow.Date.AddDays(-6);
            ...
            var summaryParams = new { canteenId, from, to = to.AddDays(1) };
            var statusClause  = hasStatus ? "AND LOWER(o.order_status) = @statusFilter" : string.Empty;

            var summary = await connection.QuerySingleAsync<ReportSummaryRow>(new CommandDefinition(
                $"""
                SELECT
                    COUNT(1)                                                          AS TotalOrders,
                    COALESCE(SUM(CASE WHEN o.order_status='completed' THEN {money} ELSE 0 END), 0) AS TotalRevenue,
                    COALESCE(AVG(CASE WHEN o.order_status='completed' THEN {money} END), 0)        AS AvgOrderValue,
                ...
                WHERE o.canteen_id = @canteenId
                  ...
                  {statusClause};
                """,
                new { canteenId, from, to = to.AddDays(1), statusFilter },
                cancellationToken: cancellationToken));

            var topItems = (await connection.QueryAsync<TopItemRow>(new CommandDefinition(
                $"""
                SELECT
                    COALESCE(oi.item_name, mi.name, 'Unknown') AS Name,
                ...
                WHERE o.canteen_id = @canteenId
                  AND o.order_status = 'completed'
                  AND o.created_at >= @from AND o.created_at < @to
                GROUP BY COALESCE(oi.item_name, mi.name, 'Unknown')
                ORDER BY Qty DESC
                LIMIT 10;
                """,
                new { canteenId, from, to = to.AddDays(1) },
                cancellationToken: cancellationToken))).ToList();
            ...
            var dailyTrend = (await connection.QueryAsync<DailyTrendRow>(new CommandDefinition(
                ...
                SELECT
                    ...
                    COUNT(1)                                                             AS Orders,
                    COALESCE(SUM(CASE WHEN o.order_status='completed' THEN {money} ELSE 0 END), 0) AS Revenue
                FROM orders o
                WHERE o.canteen_id = @canteenId
                  AND o.created_at >= @from AND o.created_at < @to
                  {statusClause}
            ...
            return Ok(Success("Reports loaded.", new
            {
                summary = new
                {
                    totalOrders    = summary.TotalOrders,
                    totalRevenue   = Math.Round(summary.TotalRevenue, 2),
                    avgOrderValue  = Math.Round(summary.AvgOrderValue, 2),
                    totalItemsSold = topItems.Sum(i => i.Qty),
                    completed      = summary.Completed,
                    cancelled      = summary.Cancelled,
                    pending        = summary.Pending,
                },
                topItems = topItems.Select(i => new
                {
                    itemName     = i.Name,
                    quantitySold = i.Qty,
                    category     = "–",
        ...
        {
            logger.LogError(ex, "GetReports failed for canteen {Id}", canteenId);
            return StatusCode(500, Failure("Internal server error."));
        }
    }
    ...
    private sealed class ReportSummaryRow
    {
